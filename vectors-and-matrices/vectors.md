...
> These properties are not incidental: together they constitute the defining axioms of a vector space. The set $\mathbb{R}^n$ equipped with these two operations forms a vector space over the [field](../fields/) $\mathbb{R}$, a structure that is examined in greater generality in the entry on [vector spaces](../vector-spaces/).
