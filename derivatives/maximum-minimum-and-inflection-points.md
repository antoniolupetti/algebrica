...
The maximum and minimum of a [function](../functions/) $f(x)$ are, respectively, the highest and lowest values the function attains within its [domain](../determining-the-domain-of-a-function/). Given a function $y = f(x)$ with domain $D,$ a point $x_0 \in D$ is a global maximum point if $f(x_0) \geq f(x)$ for every $x \in D.$ The value $f(x_0) = M$ is the global maximum of the function.
...
A function can have several local maxima and local minima, points where it attains its highest or lowest value only in a neighborhood, without necessarily being an extreme value over the entire domain.

Given a function $y = f(x)$ defined on an interval $[a, b],$ the point $x_0 \in [a, b]$ is a local maximum point if there exists a neighborhood $I$ of the point $x_0$ such that $f(x_0) \geq f(x)$ for every $x$ in the [interval](../intervals/) $I.$

![IMG. 3](svg/maximum-minimum-inflection-points-3.svg)

A sufficient criterion for recognizing a local maximum point is based on the sign of the first derivative. Consider a function $y = f(x)$ that is defined and continuous in a neighborhood of the point $x_0,$ and differentiable in the same neighborhood for every $x \neq x_0.$ Suppose that for every $x$ in the neighborhood the derivative satisfies:
...
By [Fermat's theorem](../fermat-theorem/), the local maximum and minimum points of a differentiable function, located within the interior of the domain, are stationary points. This implies that the tangent line at a point of a local maximum or minimum is parallel to the x-axis. In this case, the [derivative](../derivatives/) of the function satisfies $f'(x_0) = 0.$

## Proof of the first-derivative criterion

The sign criterion stated above follows from [Lagrange's Theorem](../lagrange-theorem/), applied separately on each side of $x_0.$ Consider the case of a local maximum, with $f$ continuous in a neighborhood of $x_0$ and differentiable there for every $x \neq x_0,$ and suppose $f'(x) > 0$ for $x < x_0$ and $f'(x) < 0$ for $x > x_0.$

Take a point $x < x_0$ in the neighborhood. On the interval $[x, x_0]$ the function is continuous, and it is differentiable in the interior, so Lagrange's Theorem gives a point $\xi \in (x, x_0)$ such that:
...
Take now a point $x > x_0$ in the neighborhood. On the interval $[x_0, x]$ the same theorem gives a point $\xi \in (x_0, x)$ such that:
...
For every $x \neq x_0$ in the neighborhood we have found $f(x) < f(x_0),$ which is the condition for $x_0$ to be a local maximum. The criterion for a local minimum is obtained by applying the same argument to $-f,$ which reverses the inequalities on the sign of the derivative.

> Continuity of $f$ at $x_0$ is what allows Lagrange's Theorem to be applied on intervals having $x_0$ as an endpoint, without assuming that the derivative exists at $x_0$ itself. For this reason the criterion recognizes corners and cusps as extrema, and not only stationary points where $f'(x_0) = 0.$
...
The analytical formulation of these concepts is discussed in the entry on [convexity and concavity of functions](../convexity-and-concavity-of-functions/).

## Inflection points and change in concavity

An inflection point is a point where the concavity of a function changes.

Consider a function $y = f(x)$ defined on an interval $(a, b),$ and let $x_0 \in (a, b)$ be either a point where $f(x)$ is differentiable, or a point where:
...
Given a continuous function, to find the local maximum and minimum points, we analyze the sign of the first derivative. The procedure is:
...
Being a [polynomial function](../polynomial-function/), it is continuous and differentiable for all $x \in \mathbb{R}.$ The first derivative of the function is:

$$f'(x) = 3x^2 - x$$

We study the sign of the derivative by imposing:
...
The point $\left( \dfrac{1}{3}, -\dfrac{1}{54} \right)$ is therefore a local minimum.
...
We return to the function of Example 1 and determine its [convexity and concavity](../convexity-and-concavity-of-functions/):
...
A point $x_0$ is an inflection point when the second derivative $f''(x)$ changes sign as $x$ crosses $x_0.$ The type of inflection depends on the behavior of the first derivative at $x_0.$
