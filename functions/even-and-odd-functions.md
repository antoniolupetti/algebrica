...
One useful consequence of evenness is the simplification it allows in [definite integrals](../definite-integrals/) over symmetric intervals. If $f(x)$ is a [continuous](../continuous-functions/) and even function, its graph is symmetric with respect to the $y$-axis.

This symmetry determines how we evaluate definite integrals over intervals of the form $[-a, a]$. The following identity holds:
...
The function $f(x) = 0$ is the only function that is both even and odd. If a function were both even and odd, it would satisfy:

+ $f(-x) = f(x)$, because it is even.
+ $f(-x) = -f(x)$, because it is odd.

Combining the two identities gives $f(x) = -f(x)$, hence $2f(x) = 0$ and $f(x) = 0$ for all $x$. The zero function is the unique case that satisfies both properties.
...
The exponential function provides the standard illustration. Its even and odd parts are the [hyperbolic cosine and sine](../hyperbolic-sine-and-cosine/):
