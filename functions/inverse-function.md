...
Consider the [power function](../power-function/) $f(x) = x^2,$ defined on $\mathbb{R}.$ This is a quadratic function, represented by a [parabola](../parabola/) with its vertex at the origin of the [Cartesian plane](../the-cartesian-coordinate-plane/). On its full domain $\mathbb{R}$ the function is not invertible, since it is not injective, because distinct inputs can produce the same output, for example $f(-2) = f(2).$
