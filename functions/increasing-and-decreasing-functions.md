...
A [function](../functions/) is classified as increasing, decreasing, or monotonic according to how its output values change as the input grows.
...
A function with domain $X \subseteq \mathbb{R}$ is strictly monotonic on an interval $I \subseteq X$ if it is either strictly increasing or strictly decreasing throughout $I,$ with no change in direction or flat segments.
...
> The [sign analysis](../sign-analysis-in-inequalities/) of a product, as in this example, examines the sign of each factor and determines the overall sign on each interval by multiplying those signs.
...
At an interior point $x_0$ both one-sided limits of an increasing function are finite and ordered as $f(x_0^-) \leq f(x_0) \leq f(x_0^+).$ Continuity at $x_0$ fails exactly when the outer terms differ, so the only [discontinuity](../discontinuities-of-real-functions/) a monotonic function can have is a jump, and the nonnegative quantity
