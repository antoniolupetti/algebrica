...
source: https://algebrica.org/polynomial-equations/
...
**Definition 1.** A polynomial equation is an equation in which one side consists of a [polynomial](../polynomials/) expression and the other is zero. The general form of such an equation is the following:
...
+ the coefficients $a_0, a_1, \ldots, a_n$ are real or [complex numbers](../complex-numbers/)
...
**Definition 2.** A root $x_0$ is said to have multiplicity $m$ if the factor $(x - x_0)^m$ divides the polynomial but $(x - x_0)^{m+1}$ does not.

+ A root of multiplicity one is called a simple root.
+ A root of multiplicity two is called a double root, and so on.

The multiplicity of a root has a [geometric interpretation](../roots-of-a-polynomial/). A simple root corresponds to a transversal crossing of the graph of the polynomial with the horizontal axis, a root of even multiplicity corresponds to a tangency point where the graph touches but does not cross the axis, and a root of odd multiplicity greater than one corresponds to a crossing at which the graph flattens against the axis before passing through it.
...
Since $x = 3$ is a root, the factor $(x - 3)$ divides the polynomial. Performing the [polynomial division](../polynomial-division/) yields the following factorisation:
...
The coefficients of a polynomial equation are not independent of its roots: they are related to them through a set of identities known as [Vieta's formulas](../vieta-formulas/). For a monic polynomial equation of degree $n$ with roots $x_1, x_2, \ldots, x_n$, these identities take the following form:
...
Each coefficient is therefore an elementary symmetric polynomial in the roots. The derivation and a detailed discussion of the quadratic case are given in the entries on [Vieta's formulas](../vieta-formulas/) and [trinomials](../trinomials/).

## Reduction to a polynomial equation

Many equations that do not appear polynomial at first sight can be reduced to polynomial form through algebraic manipulation, after which the methods discussed in this entry apply directly. A [rational equation](../rational-equations/) such as the following:

$$
x + \frac{1}{x} = 3
$$

becomes polynomial upon multiplying both sides by $x$, yielding $x^2 - 3x + 1 = 0$. An [irrational equation](../irrational-equations/) such as the following:
...
For polynomial equations of degree up to four, explicit formulas expressing the roots in terms of the coefficients by means of arithmetic operations and [radicals](../radicals/) are known. The [quadratic formula](../quadratic-formula/) handles degree two. Analogous but considerably more involved formulas, due to Cardano and Ferrari, handle degrees three and four respectively.

For degree five and beyond, no such general formula exists. This result, established rigorously by Abel and [Ruffini](../synthetic-division/) and placed within a definitive theoretical framework by Galois, is one of the landmark theorems of modern algebra.
...
To appreciate what this means, consider the following equation:
...
and its five roots are the [fifth roots of unity](../roots-of-unity/), which can be written explicitly in terms of radicals and trigonometric values. Now consider the following equation:
