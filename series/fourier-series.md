...
The definition of the Fourier series does not by itself guarantee convergence to the original function. A sufficient condition requires that $f$ be periodic and that $f$ together with its derivative $f'$ be [piecewise continuous](../piecewise-functions/) on a period. Concretely, on $[-\pi,\pi]$ the function should satisfy:
