...
A further classification, transversal to this hierarchy, partitions $\mathbb{R}$ into the algebraic numbers $\mathbb{A}$ and the transcendental ones, and refines the distinction between rational and irrational. Each extension resolves a limitation of the previous one, until $\mathbb{C}$ is reached, within which every [polynomial equation](../polynomial-equations/) has a solution.
...
Each element is obtained by adding one to the previous, starting from $0$. Because natural numbers express how many elements a collection contains, they are also called cardinal numbers. Whether zero belongs to $\mathbb{N}$ is a matter of convention; the two most common choices are the following:
...
The passage from $\mathbb{N}$ to $\mathbb{Z}$ makes subtraction always well-defined: for any $a, b \in \mathbb{Z}$ the difference $a - b$ is again an integer. A subtraction such as $3 - 5$, which has no value inside $\mathbb{N}$, produces the integer $-2$ in $\mathbb{Z}$. From an algebraic standpoint, $(\mathbb{Z}, +)$ is an abelian [group](../groups/) and $(\mathbb{Z}, +, \cdot)$ is a commutative [ring](../rings/) with unity, although not a field, since most integers have no multiplicative inverse. A dedicated entry covers the properties of [integers](../integers/) in detail.

## Rational numbers

The set of [rational numbers](../rational-numbers/), denoted by $\mathbb{Q}$, is the collection of all numbers that can be expressed as a ratio of two integers with a nonzero denominator:
...
The irrationality of $\sqrt{2}$ is one of the oldest results in mathematics and has a concise proof by contradiction. Assuming $\sqrt{2} = p/q$ in lowest terms forces both $p$ and $q$ to be even, which contradicts the assumption. 

The numbers $\pi$ and $e$ are irrational and also transcendental, that is, they are not roots of any nonzero polynomial with rational coefficients.
...
The passage from $\mathbb{Q}$ to $\mathbb{R}$ fills the gaps left by the rationals, so that every convergent [sequence](../sequences/) has a [limit](../limits/) within the set. Every real number has a decimal representation of the form:
...
+ The digits are indexed by the natural numbers, so the decimal expansion continues indefinitely.
...
Geometrically, $\mathbb{R}$ corresponds to the points of a straight line with no gaps, the real number line.

Completeness distinguishes $\mathbb{R}$ from $\mathbb{Q}$. The sequence of rational approximations to $\sqrt{2}$, for instance, has no limit within $\mathbb{Q}$, but its limit exists in $\mathbb{R}$. The topic of least upper bounds is treated in the entry on [supremum and infimum](../supremum-and-infimum/).

The set $\mathbb{R}$ is totally ordered: for any two real numbers $x$ and $y$, exactly one of the relations $x < y$, $x = y$, or $x > y$ holds. $\mathbb{R}$ satisfies the Archimedean property: for every real number $x$ there exists a natural number $n$ such that $n > x$. The property excludes infinitely large and infinitely small elements from $\mathbb{R}$.
...
The set of [complex numbers](../complex-numbers/), denoted by $\mathbb{C}$, extends $\mathbb{R}$ by introducing an element $i$ satisfying $i^2 = -1$. Every complex number has a unique decomposition into a real and an imaginary component, written as follows:
...
Geometrically, $\mathbb{C}$ is identified with the Euclidean plane by representing $z = a + bi$ as the point of coordinates $(a, b)$. The horizontal axis corresponds to the real numbers and the vertical axis to the purely imaginary ones, and the algebraic operations have a direct geometric interpretation.
...
The set $\mathbb{R}$ does not have this closure property, since the polynomial $x^2 + 1$ has no real roots. A full treatment is given in the entry on [complex numbers](../complex-numbers/).
