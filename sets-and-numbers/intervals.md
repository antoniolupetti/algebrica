...
Intervals are among the most fundamental objects in mathematical analysis. They appear as domains of functions, as regions of integration in the theory of [definite integrals](../definite-integrals/), as sets on which continuity and differentiability are studied, and as the building blocks for describing more complex subsets of the [real line](../real-numbers/).

Intervals are classified by whether their endpoints are included or excluded, and by whether they are bounded or extend indefinitely in one or both directions.
...
An unbounded interval extends indefinitely in at least one direction. Since infinity is not a real number, the symbols $+\infty$ and $-\infty$ are used purely as notational conventions to indicate that the interval has no finite bound in the corresponding direction. The endpoints $+\infty$ and $-\infty$ are always excluded, and the corresponding bracket is always a parenthesis. The four unbounded intervals are defined as follows.
...
Given two intervals, one may form new sets by combining them through the standard set-theoretic operations of intersection and union, defined in the entry on [sets](../sets/). The intersection $I \cap J$ is the set of all points belonging to both intervals simultaneously. The intersection of two intervals is always an interval, possibly empty or degenerate. Consider $I = (1, 5)$ and $J = (3, 7)$. The values belonging to both are precisely those in $(3, 5)$.
...
The third row shows the intersection $(3, 5)$, which is the portion shared by both intervals.

- - -

The union $I \cup J$ is the set of all points belonging to at least one of the two intervals. Unlike intersection, the union of two intervals is not always an interval. It is an interval if and only if the two intervals overlap or share an endpoint. Consider the same example, $I = (1, 5)$ and $J = (3, 7)$. Since the two intervals overlap, their union is the interval $(1, 7)$.
...
The third row shows the union $(1, 7)$. By contrast, the union $(1, 3) \cup (5, 7)$ is not an interval, because the points between $3$ and $5$ belong to neither set.

## Intervals and neighborhoods

A concept closely related to intervals and central to mathematical analysis is that of a neighborhood of a point. Given a point $x_0 \in \mathbb{R}$ and a real number $\varepsilon > 0$, the open interval:

$$
(x_0 - \varepsilon, \ x_0 + \varepsilon)
$$

is called the $\varepsilon$-neighborhood of $x_0$, or simply a neighborhood of $x_0$. It consists of all points whose distance from $x_0$ is strictly less than $\varepsilon$, that is, all $x$ satisfying $|x-x_0| < \varepsilon$, where $|\cdot|$ denotes the [absolute value](../absolute-value/).
...
The length of a bounded interval with endpoints $a$ and $b$ is defined as $b - a$, regardless of whether the endpoints are included or excluded. The four intervals $(a, b)$, $[a, b)$, $(a, b]$, and $[a, b]$ all have the same length, given by the following expression:
...
A subset of the real line is said to be connected if it cannot be written as the union of two disjoint non-empty open sets. The following theorem provides a complete characterization of intervals in terms of this property. A subset $S \subseteq \mathbb{R}$ is an interval if and only if it is connected.

The result makes precise the intuitive idea that an interval is a portion of the real line with no gaps. The condition of connectedness rules out sets such as $(1, 2) \cup (3, 4)$, which fail to be intervals because they can be separated into two disjoint open pieces.
