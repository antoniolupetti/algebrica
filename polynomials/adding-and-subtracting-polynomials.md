...
source: https://algebrica.org/adding-and-subtracting-polynomials/
...
Let $R$ be a commutative [ring](../rings/) and $R[x]$ the ring of polynomials in one indeterminate over $R$. Consider two [polynomials](../polynomials/) $P(x)$ and $Q(x)$, each written as a sum of [monomials](../monomials/), where any missing coefficients are understood to be zero:
...
If $n \neq m$, the leading term of the polynomial of higher degree has no counterpart to cancel against, and the degree of the result is exactly $\max\\{n, m\\}$. When $n = m$, the degree may decrease: under addition, the leading term cancels whenever $a_n + b_n = 0$; under subtraction, whenever $a_n = b_n$. If every term cancels, the result is the zero polynomial, to which the convention $\deg 0 = -\infty$ is assigned, in keeping with the definition of the [degree of a polynomial](../polynomials/).
...
A further property concerns the interaction with multiplication. [Polynomial multiplication](../multiplying-polynomials/) distributes over addition, both on the left and on the right:
