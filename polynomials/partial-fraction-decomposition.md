...
source: https://algebrica.org/partial-fraction-decomposition/
...
so that $A = 1/2$. Setting $x = 2$ gives:
...
Substituting the value $C = -1/9$ found above gives $A = 1/9$.
...
> This example shows how the partial fraction method handles an irreducible quadratic factor: the corresponding numerator is taken as a generic polynomial of degree one, and its coefficients are recovered by comparing the coefficients of equal powers of $x$ on both sides of the polynomial identity.
