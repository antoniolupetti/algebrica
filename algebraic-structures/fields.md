...
A field is an algebraic structure in which the operations of addition and multiplication are both fully invertible, subject to the sole exception that division by zero is excluded. The concept arises when one observes that certain number systems, such as the [rational numbers](../rational-numbers/), the [real numbers](../real-numbers/), and the [complex numbers](../complex-numbers/), allow addition, subtraction, multiplication, and division by any nonzero element, with all the expected algebraic rules in force.
...
+ Multiplication distributes over addition: for all $a, b, c \in F,$ the identity $a \cdot (b + c) = a \cdot b + a \cdot c$ holds.
...
Several properties follow directly from the previous axioms. For any $a \in F,$ multiplication by zero satisfies $a \cdot 0 = 0.$ This is not assumed but derived:

$$a \cdot 0 = a \cdot (0 + 0) = a \cdot 0 + a \cdot 0$$

Cancelling $a \cdot 0$ from both sides through the additive group structure produces the result.

A field contains no zero divisors. If $a \cdot b = 0$ and $a \neq 0,$ then $a$ is invertible and:

$$b = a^{-1} \cdot (a \cdot b) = a^{-1} \cdot 0 = 0$$

In a field the product of two nonzero elements is therefore always nonzero, which is the property that makes cancellation possible throughout algebra. The additive and multiplicative structures interact through the identity:

$$(-a) \cdot b = a \cdot (-b) = -(a \cdot b)$$

The expression holds for all $a, b \in F.$ In particular, the product of two negative elements is positive in the sense that:
...
A field is obtained by imposing one further requirement on a commutative ring with unity, namely that every nonzero element be invertible with respect to multiplication. The three structures form a chain of increasing rigidity:
...
> The rational numbers $\mathbb{Q},$ the real numbers $\mathbb{R},$ and the complex numbers $\mathbb{C}$ are all fields. The integers $\mathbb{Z},$ by contrast, form a ring but not a field, since division does not close within them. Fields play a central role in linear algebra, where they provide the scalars for [vector spaces](../vector-spaces/); when the scalars are drawn from a ring rather than a field, the resulting structure is a [module](../modules/).
...
A subset $K \subseteq F$ is called a subfield of $F$ when $K$ is itself a field under the operations inherited from $F.$ Equivalently, $K$ is a subfield of $F$ when it contains $0$ and $1$ and is closed under addition, negation, multiplication, and taking multiplicative inverses of nonzero elements. The rational numbers $\mathbb{Q}$ form a subfield of $\mathbb{R},$ which is itself a subfield of $\mathbb{C}.$ These inclusions define a chain of fields:

$$\mathbb{Q} \subseteq \mathbb{R} \subseteq \mathbb{C}$$

When $K$ is a subfield of $F,$ the field $F$ is called a field extension of $K,$ written $F/K.$ From this perspective, $\mathbb{C}/\mathbb{R}$ is a field extension, and $\mathbb{C}$ can be studied as a two-dimensional [vector space](../vector-spaces/) over $\mathbb{R}$ with basis $\\{\ 1, i \ \\}.$ The dimension of $F$ regarded as a vector space over $K$ is called the degree of the extension and is denoted $[F : K].$ In this example, $[\mathbb{C} : \mathbb{R}] = 2.$
...
is an [ideal](../rings/) of $F.$ Since $F$ is a field, its only ideals are $\\{\ 0 \ \\}$ and $F$ itself, and the condition $\varphi(1) = 1 \neq 0$ rules out the second possibility. A bijective field homomorphism is called a field isomorphism. Two fields are isomorphic, written $F \cong K,$ when an isomorphism between them exists. Isomorphic fields are algebraically indistinguishable: they share all properties that depend only on the field axioms.

> A function is injective, or one-to-one, when distinct elements of the domain are mapped to distinct elements of the codomain: $\varphi(a) = \varphi(b)$ implies $a = b.$ A function is bijective when it is both injective and surjective, meaning it is one-to-one and every element of the codomain is the image of at least one element of the domain. The same notions of homomorphism, kernel, and isomorphism reappear, with the appropriate adjustments, in the theory of [groups](../groups/), [rings](../rings/), and [modules](../modules/). A unified treatment is collected on the page about [homomorphisms and isomorphisms](../homomorphisms-and-isomorphisms/).
