...
> Finite fields exist only when the number of elements is a prime power $p^n$, for some prime $p$ and positive integer $n$. For every such prime power there exists, up to isomorphism, exactly one finite field, denoted $\mathbb{F}_{p^n}$ or $\operatorname{GF}(p^n)$.
