...
> Fermat's Theorem plays a key role in the proof of [Rolle's Theorem](../rolle-theorem/), which in turn underlies the proof of [Lagrange's Theorem](../lagrange-theorem/) and, ultimately, [Cauchy's Theorem](../cauchy-theorem/) and [L'Hopital's rule](../hopital-rule/). The existence of the extremum required by Fermat's statement is itself guaranteed, on a closed and bounded interval, by [Weierstrass' Theorem](../weierstrass-theorem/).
...
Fermat's theorem singles out the candidates for an interior extremum but is silent on their nature. The classification is recovered by examining the second [derivative](../derivatives/) of $f$ at the stationary point. Let $f$ be twice differentiable in a neighbourhood of an interior point $x_0$ with $f'(x_0) = 0$. The sign of $f''(x_0)$ determines the local behaviour of the function as follows:
...
- - -
The third case, $f''(x_0) = 0$, leaves the second-order term inert and requires examining the higher-order derivatives. The general principle is that the nature of the stationary point is governed by the first non-vanishing derivative of $f$ at $x_0$. If $f^{(k)}(x_0)$ is the first such derivative with $k \geq 2$, then:
...
> The second-derivative test is a local sufficient condition. It complements Fermat's theorem, which provides only the necessary condition $f'(x_0) = 0$, by furnishing a decision rule that classifies most stationary points encountered in practice.
